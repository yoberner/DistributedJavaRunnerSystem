...
public class ZooKeeperPeerServerImpl extends Thread implements ZooKeeperPeerServer {
	...
	public ZooKeeperPeerServerImpl(int myPort, long peerEpoch, Long id, Map<Long, InetSocketAddress> peerIDtoAddress) {
		...
		this.incomingGossipMessages = new LinkedBlockingQueue<Message>();
	...
	public void shutdown() {
		...
		if (this.heartbeat != null) this.heartbeat.shutdown();
	...
	public void run() {
		...
		try {
			this.receiverWorker = new UDPMessageReceiver(this.incomingGossipMessages, this.incomingMessages, this.myAddress, this.myPort, this);
			...
			while (!this.shutdown) {
				...
				case LEADING:
					...
					if (this.heartbeat == null) {
						this.heartbeat = new Heartbeat(peerIDtoAddress, incomingGossipMessages, outgoingMessages, this);
